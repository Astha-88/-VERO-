  ...
  return (
    <div className="app-shell">
      ...
      <main className="page-content">
        ...
        {route.kind === "vehicle" && (
          <VehiclePage vehicleId={route.id} />
        )}
  ...
  const submit = async () => {
    ...
    try {
      const response = await fetch(
        `${API_BASE_URL}/vehicles?registration_number=${encodeURIComponent(
          registration,
        )}`,
      );
      ...
      if (!response.ok) {
        throw new Error(
          response.status === 404
            ? "Vehicle not found."
            : "Unable to check this vehicle right now.",
        );
      }
      ...
      const vehicle =
        vehicles.find(
          (item) =>
            item.registration_number.toUpperCase() === registration,
        ) ?? vehicles[0];
    ...
    <>
      ...
      <section id="how-it-works" className="features-section">
        ...
        <div className="feature-grid">
          ...
          >
            Core information about the vehicle, including make, model,
            variant, year, and fuel type.
          </Feature>
          ...
          >
            Understand how many owners the vehicle has had and how its
          ...
          >
            Review recorded maintenance, accidents, incidents, repairs,
            costs, and other warning signals.
          </Feature>
          ...
          >
            VERO combines the available history into an
            easy-to-understand vehicle risk assessment.
          </Feature>
        </div>
      </section>
    </>
...
function CheckPage() {
  ...
  return (
    <section className="inner-page check-page">
      ...
      <p className="lead">
        Enter the registration number to retrieve the vehicle&apos;s
        recorded history and VERO risk assessment.
      </p>
...
function VehiclePage({ vehicleId }: { vehicleId: number }) {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        const response = await fetch(
          `${API_BASE_URL}/vehicles/${vehicleId}/report`,
        );
        ...
        if (!response.ok) {
          throw new Error(
            response.status === 404
              ? "Vehicle not found."
              : "Could not load the vehicle report.",
          );
        }

        const payload = await response.json();

        const vehiclePayload = normalizeProfile(
          payload.vehicle,
          vehicleId,
        );
  	...
  	risk_assessment:
        ...
        try {
          ...
          const aiResponse = await fetch(
            `${API_BASE_URL}/vehicles/${vehicleId}/ai-summary`,
          );
          ...
          const aiData: AISummary = await aiResponse.json();
          ...
          if (!cancelled) {
            setAiError(
              aiRequestError instanceof Error
                ? aiRequestError.message
  ...
  if (!report) {
    return (
      <ErrorState message="The vehicle report could not be loaded." />
    );
  }

  const profile = report.vehicle;
      ...
      <div className="results-grid">
        <InfoCard title="Vehicle details">
          ...
          <DataRow
            ...
            value={details?.manufacturing_year?.toString()}
          />
        ...
        <InfoCard title="Risk assessment">
          ...
          <div className="signal-columns">
            <SignalList
              title="Red flags"
              items={risk.red_flags}
              negative
            />

            <SignalList
              title="Positive signals"
              items={risk.positive_signals}
            />
          </div>
        </InfoCard>
      </div>
  ...
  {rcEvidence ? (
    <>
     ...
     <InfoCard title="AI assessment">
        ...
        {aiSummary && (
          <>
            ...
            <div className="ai-summary">
              ...
              <p>{aiSummary.risk_explanation}</p>
            </div>
            ...
            <div className="ai-grid">
              ...
              <div>
                ...
                {aiSummary.positive_signals.length > 0 ? (
                  <ul>
                    {aiSummary.positive_signals.map(
                      (item, index) => (
                        <li key={index}>{item}</li>
                      ),
                    )}
                  ...
                  <p>
                    No positive signals identified from the
                    supplied evidence.
                  </p>
                )}
              </div>
              ...
              <div>
                ...
                {aiSummary.buyer_advice.length > 0 ? (
                  <ul>
                    {aiSummary.buyer_advice.map(
                      (item, index) => (
                        <li key={index}>{item}</li>
                      ),
                    )}
                  ...
                  <p>No specific buyer advice available.</p>
                )}
              </div>
              ...
              <div>
                ...
                {aiSummary.data_limitations.length > 0 ? (
                  <ul>
                    {aiSummary.data_limitations.map(
                      (item, index) => (
                        <li key={index}>{item}</li>
                      ),
                    )}
                  ...
                  <p>No major data limitations identified.</p>
                )}
              </div>
            </div>
          </>
      ...
      <InfoCard
        title={`Ownership history · ${ownership.length} recorded`}
      >
        {ownership.length ? (
          <div className="timeline">
            {ownership.map((owner) => (
              ...
              >
                ...
                <small>
                  {formatDate(owner.purchase_date)}
                  {owner.transfer_date
                    ? ` → ${formatDate(
                        owner.transfer_date,
                      )}`
                    : " → Current/last recorded owner"}
                </small>
              </div>
            ))}
          </div>
        ) : (
          <EmptyState text="No ownership records are currently available." />
        )}
      </InfoCard>

      <InfoCard
        title={`Service records · ${services.length} recorded`}
      >
        {services.length ? (
          ...
          <EmptyState text="No service records are currently available." />
        )}
      </InfoCard>

      <InfoCard
        title={`Incidents · ${incidents.length} recorded`}
      >
        {incidents.length ? (
          <div className="record-list">
            {incidents.map((incident) => (
              ...
              >
                ...
                <strong>
                  {formatMoney(incident.repair_cost)}
                </strong>
              </div>
            ))}
          </div>
        ) : (
          <EmptyState text="No incidents have been reported." />
        )}
      </InfoCard>
    </section>
  );
...
function AboutPage() {
  return (
    <section className="inner-page about-page">
      <p className="eyebrow">ABOUT VERO</p>

      <h1>Know what you&apos;re buying.</h1>

      <p className="lead">
        VERO is a vehicle due-diligence experience designed
        to bring a used car&apos;s recorded history into one
        clear picture before a buyer commits.
      </p>

      <div className="about-grid">
        <InfoCard title="What we bring together">
          <p>
            Vehicle details, ownership history, service
            records, incidents, repair information, and a
        ...
        <InfoCard title="Why it matters">
          <p>
            A used vehicle can look perfect on the outside
            while its history tells a very different story.
            VERO helps you inspect the information that
...
function NotFoundPage() {
  return (
    <section className="state-page">
      ...
      <p className="lead">
        The route you entered isn&apos;t part of VERO.
      </p>
...
function normalizeProfile(
  ...
  const vehicle = isRecord(root.vehicle)
    ? root.vehicle
    : root;

  return {
    vehicle: {
      id: Number(vehicle.id ?? vehicleId),
      registration_number: String(
        vehicle.registration_number ?? "Unknown",
      ),
    },

    details: isRecord(root.details)
      ? (root.details as VehicleDetails)
      : null,

    ownership: Array.isArray(root.ownership)
      ? (root.ownership as Ownership[])
      : [],

    service_records: Array.isArray(
      root.service_records,
    )
      ? (root.service_records as ServiceRecord[])
      : [],

    incidents: Array.isArray(root.incidents)
      ? (root.incidents as Incident[])
      : [],
  };
}
